...
import { getKnowledgeEvidence } from "@/lib/knowledge-service"
...
export async function GET(request: Request, context: { params: Promise<{ sourceId: string }> }) {
  ...
  const result = await getKnowledgeEvidence(sourceId, {
    entityKey,
    slug,
    limit,
  })
  ...
  if (!result) {
    return NextResponse.json(
      {
        status: "not_found",
        message: "No evidence found for the provided input",
      },
      { status: 404 },
    )
  }
